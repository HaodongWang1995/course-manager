...
- [x] 新增路由 `/teacher/students/:studentId`，教师可从学生列表点击进入
- [x] PC 端两栏布局：左侧 Performance Overview + Recent Submissions，右侧 Enrolled Courses + Private Notes
- [x] H5 端单栏布局：头部居中头像 → Performance Overview → Enrolled Courses → Teacher's Private Notes → Recent Submissions
- [x] 新增 API 端点 `GET /api/teachers/students/:studentId` 返回学生详情（含已选课程列表）
- [x] 所有 UI 文本支持 i18n（en + zh）
- [x] 学生列表中的"查看详情"链接正确跳转到 `/teacher/students/:studentId`
- [x] 现有测试不受影响（`pnpm test` 全部通过）
