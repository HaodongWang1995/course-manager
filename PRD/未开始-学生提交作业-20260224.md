...
- [ ] 学生作业卡片有「Submit Assignment」按钮，点击后状态变为 Submitted
- [ ] 提交时可选上传文件（复用 FileUploadZone），文件存储到 R2/本地
- [ ] 已提交的作业显示 Submitted 状态，按钮变为禁用或显示「已提交」
- [ ] 教师端作业列表每项显示「X/Y submitted」（X 已提交，Y 总选课人数）
- [ ] hover 已提交数字时 Tooltip 显示已提交学生姓名列表
- [ ] hover 未提交数字时 Tooltip 显示未提交学生姓名列表
- [ ] i18n 覆盖所有新增文案（中/英）
...
- [ ] 数据库：创建 005_submission_attachments.sql 迁移
- [ ] 后端：修改 PATCH /api/assignments/:id/submit 支持 file_key 参数
- [ ] 后端：新增 GET /api/assignments/:id/submissions 端点
- [ ] 后端：attachments presign/upload 支持 submission_id
- [ ] 前端：学生作业页添加提交对话框（含 FileUploadZone）
- [ ] 前端：提交后状态更新为 Submitted，按钮禁用
- [ ] 前端：教师作业区域显示「X/Y submitted」统计
- [ ] 前端：hover 数字弹出 Tooltip 显示学生姓名列表
- [ ] 前端：新增 useAssignmentSubmissions hook
- [ ] i18n：新增提交相关翻译 key
- [ ] 测试：为新增/修改的 API 编写集成测试
